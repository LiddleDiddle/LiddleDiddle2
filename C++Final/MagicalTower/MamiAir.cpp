...
CharacterState* MamiAir::update(b2Body & body, int controllerNumber, int contacts){
	if (contacts == 0)
	{
...
void MamiAir::draw(Bengine::SpriteBatch& spriteBatch, b2Body *body){

	Bengine::ColorRGBA8 color(255, 255, 255, 255);

	if (body->GetLinearVelocity().y > 0)
		_texture = Bengine::ResourceManager::getTexture("Textures/Mami/jump.png");
	uv = glm::vec4(0, 0, 1, 1);

	if (body->GetLinearVelocity().y <= 0)
	{
		_texture = Bengine::ResourceManager::getTexture("Textures/Mami/fall.png");
		uv = glm::vec4(0, 0, 1, 1);
	}
...
void MamiAir::processInputs(int controllerNumber){
