...
CharacterState* HomuraAir::update(b2Body & body, int controllerNumber, int contacts){
	if (contacts == 0)
	{
...
void HomuraAir::draw(Bengine::SpriteBatch& spriteBatch, b2Body *body){

	Bengine::ColorRGBA8 color(255, 255, 255, 255);

	if (body->GetLinearVelocity().y > 0)
		_texture = Bengine::ResourceManager::getTexture("Textures/Homura/jump.png");
	uv = glm::vec4(0, 0, 1, 1);

	if (body->GetLinearVelocity().y <= 0)
	{
		_texture = Bengine::ResourceManager::getTexture("Textures/Homura/fall.png");
		uv = glm::vec4(0, 0, 1, 1);
	}
...
void HomuraAir::processInputs(int controllerNumber){

}

void HomuraAir::enter(){
